...
function Out-Result([string]$status, [string]$message = "") {
  @{ tweak = "Disable Remote Registry"; status = $status; message = $message } | ConvertTo-Json -Compress
}

$ServiceName = "RemoteRegistry"

function Get-ServiceState {
  $service = Get-CimInstance Win32_Service -Filter "Name='RemoteRegistry'" -ErrorAction Stop
...
try {
  switch ($State) {
    ...
    'Off' {
      Set-ServiceState -Name $ServiceName -StartupType "Manual" -Action "Start"
      Out-Result "Disabled"
    }
  }
