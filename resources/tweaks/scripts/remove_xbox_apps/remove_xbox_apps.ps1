...
function Out-Result([string]$status, [string]$message = "") {
  @{ tweak = "Remove Xbox Apps"; status = $status; message = $message } | ConvertTo-Json -Compress
}
...
try {
  switch ($State) {
    'Check' {
      ...
      if ($totalCount -eq 0) {
        Out-Result "Enabled" "No Xbox-related packages were found"
      }
      else {
        Out-Result "Disabled" "$totalCount Xbox-related package(s) detected"
      }
  ...
  if (-not (Test-IsAdministrator)) {
    throw "Administrator privileges are required to remove Xbox apps."
  }
