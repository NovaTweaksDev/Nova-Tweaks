...
$TweakName = "Nova Tweaks Performance Power Plan"
$PlanName  = "Nova Tweaks Performance"
...
function Get-PlanDescription {
  ...
  return "$cpuName - Performance profile with aggressive boost behavior, reduced core parking and lower energy-saving interruptions."
}
...
try {
  switch ($State) {
    'Check' {
      ...
      if ($plan -and ($active -eq $plan)) { Out-Result "Enabled" }
      else { Out-Result "Disabled" }
    }
    ...
    'On' {
      ...
      if ($exists) {
        ...
        $baseGuid = if (powercfg /list | Select-String -SimpleMatch $guidUltimate) { $guidUltimate } else { $guidHigh }
