...
try {
  switch ($State) {
    ...
    'On' {
      $result = Invoke-IpConfig -Arguments @('/renew')

      if ($result.ExitCode -ne 0) {
        throw "ipconfig /renew failed with exit code $($result.ExitCode)"
      }

      Out-Result "Enabled" "IP configuration renewed"
    }
