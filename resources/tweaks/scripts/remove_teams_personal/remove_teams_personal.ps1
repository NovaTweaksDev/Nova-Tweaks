...
function Out-Result([string]$status, [string]$message = "") {
  @{ tweak = "Remove Teams (Personal)"; status = $status; message = $message } | ConvertTo-Json -Compress
}
...
try {
  switch ($State) {
    'Check' {
      ...
      if ($totalCount -eq 0) {
        Out-Result "Enabled" "Microsoft Teams Personal was not found"
      }
      else {
        Out-Result "Disabled" "$totalCount Microsoft Teams Personal package(s) detected"
      }
  ...
  if (-not (Test-IsAdministrator)) {
    throw "Administrator privileges are required to remove Microsoft Teams Personal."
  }
