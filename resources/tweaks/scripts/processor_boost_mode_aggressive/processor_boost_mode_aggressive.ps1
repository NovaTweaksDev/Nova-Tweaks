...
function Out-Result([string]$status, [string]$message = "") {
  @{ tweak = "Processor Boost Mode (Aggressive)"; status = $status; message = $message } | ConvertTo-Json -Compress
}
...
function Get-SettingValue {
  ...
  if ($values.Count -lt 2) {
    throw "Failed to read Processor Boost Mode values."
  }
...
try {
  switch ($State) {
    'Check' {
      ...
      if (($current.AC -eq 2) -and ($current.DC -eq 2)) {
        Out-Result "Enabled"
      }
      else {
        Out-Result "Disabled"
