...
function Out-Result([string]$status, [string]$message = "") {
  @{ tweak = "Remove Clipchamp"; status = $status; message = $message } | ConvertTo-Json -Compress
}
...
try {
  switch ($State) {
    'Check' {
      ...
      if ($totalCount -eq 0) {
        Out-Result "Enabled" "Clipchamp was not found"
      }
      else {
        Out-Result "Disabled" "$totalCount Clipchamp package(s) detected"
      }
  ...
  if (-not (Test-IsAdministrator)) {
    throw "Administrator privileges are required to remove Clipchamp."
  }
