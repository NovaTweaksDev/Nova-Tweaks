...
try {
  switch ($State) {
    ...
    'On' {
      ...
      $rawOutput = & $dismPath '/Online' '/Cleanup-Image' '/StartComponentCleanup' 2>&1
      ...
      switch ($exitCode) {
        ...
        default {
          ...
          $errorMessage = if ($adminRequired) {
            "Administrator rights are required to clean up the Component Store"
          } else {
