...
try {
  switch ($State) {
    ...
    'On' {
      ...
      $rawOutput = & $dismPath '/Online' '/Cleanup-Image' '/AnalyzeComponentStore' 2>&1
      ...
      $summaryLine = $outputLines | Where-Object {
        $_ -match '(?i)component store|operation completed successfully'
      } | Select-Object -Last 1
      $summary = if ($summaryLine) {
        $summaryLine
      } else {
        "Component Store analysis completed"
      }
      $details = @{
      ...
      switch ($exitCode) {
        ...
        default {
          ...
          $errorMessage = if ($adminRequired) {
            "Administrator rights are required to analyze the Component Store"
          } else {
